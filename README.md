...
|---|---|
| ROC-AUC | 0.7804 |
| PR-AUC | 0.2765 (baseline/no-skill: 0.0807) |
| Recall @ 0.5 threshold (class 1 / default) | 66.7% |
| Precision @ 0.5 threshold (class 1 / default) | 18.9% |
...
**Feature importance validates the multi-table approach**: engineered features from `bureau` (`bureau_total_credit`, `bureau_debt_credit_ratio`, `bureau_max_overdue_amt`), `previous_application` (`prev_avg_annuity`, `prev_avg_credit_amt`), and `pos_cash_balance` (`pos_months_count`) all rank among the top 20 most important features — alongside the dataset's well-known `EXT_SOURCE_1/2/3` external credit scores. `credit_card_balance` and `bureau_balance` features don't crack the top 20 by gain-based importance, but `cc_max_utilization` (credit card utilization) surfaced independently as a genuine, statistically-supported business rule — see section 8 — demonstrating that "important to the model" and "useful signal for a specific decision rule" aren't always the same thing, which is itself part of the case for pairing a black-box model with an explicit rule-derivation step.
...
- If cc_max_utilization in [1.04, 11.78], default rate is 12.7% vs 8.1% baseline (1.57x higher risk, n=21,509).
...
- **The Risk Prediction UI form** exposes ~12 key fields and fills the rest from population medians/modes, rather than all 165 model features — a deliberate UX tradeoff for a demo interface; a production underwriting system would pull the full applicant record automatically rather than via manual form entry.
